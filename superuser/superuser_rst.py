...
namespace = MUBNamespace("superuser", sessionmaker=sessionmaker, path="")
...
class PermissionIndex(Resource):
    @permission_index.require_permission(namespace, manage_mods, use_moderator=False)
    @namespace.marshal_list_with(Permission.IndexModel)
    def get(self, session):
        return Permission.get_all(session)


@namespace.route("/moderators/")
class ModeratorIndex(Resource):
    @permission_index.require_permission(namespace, manage_mods)
    @namespace.argument_parser(search_counter_parser)
    @namespace.lister(100, Moderator.IndexModel)
    ...
    @namespace.doc_abort(400, "Moderator with is username already exists")
    @permission_index.require_permission(namespace, manage_mods)
    @namespace.argument_parser(parser)
    @namespace.marshal_with(Moderator.IndexModel)
    def post(self, session, moderator: Moderator, username: str, password: str, append_perms: list[int]):
        ...
        if Moderator.find_by_name(session, username) is not None:
            namespace.abort(400, "Moderator with is username already exists")
...
@namespace.route("/moderators/<int:moderator_id>/")
class ModeratorManager(Resource):
    ...
    @namespace.doc_abort(400, "Target is the source")
    @namespace.doc_abort(400, "Can't edit superuser's permissions")
    @namespace.doc_abort(403, "Insufficient permissions")
    @namespace.doc_abort(404, "Permission not found")
    @permission_index.require_permission(namespace, manage_mods)
    @namespace.database_searcher(Moderator, result_field_name="target", use_session=True)
    @namespace.argument_parser(parser)
    def post(self, session, moderator: Moderator, target: Moderator, username: str | None, password: str | None,
        ...
        if moderator.id == target.id:
            namespace.abort(400, "Target is the source")
        if target.superuser:
            namespace.abort(400, "Can't edit superuser's permissions")
    ...
    @namespace.doc_abort(400, "Target is the source")
    @namespace.doc_abort(403, "Can't delete a superuser via web api")
    @permission_index.require_permission(namespace, manage_mods)
    @namespace.database_searcher(Moderator, result_field_name="target", use_session=True)
    def delete(self, session, moderator: Moderator, target: Moderator):
        if moderator.id == target.id:
            namespace.abort(400, "Target is the source")
        if target.superuser:
            namespace.abort(403, "Can't delete a superuser via web api")
